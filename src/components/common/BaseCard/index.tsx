...
export function BaseCard(props: IBaseCardProps) {
  const { isDisabled = false, listType, status, sx, children, title, onOk, onCancel, okText, cancelText } = props
  return (
    <Card sx={sx}>
      <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
        ...
        {(!!onOk || !!onCancel) && !isDisabled && (
          <Stack direction='row' justifyContent='flex-end' gap={2}>
            {!!onOk && <StyledButtonDefault onClick={onOk}>{okText || 'confirm'}</StyledButtonDefault>}
            {!!onCancel && <StyledButtonCancel onClick={onCancel}>{cancelText || 'cancel'}</StyledButtonCancel>}
          </Stack>
        )}
        {isDisabled && (
          <Stack direction='row' justifyContent='flex-end' gap={2}>
